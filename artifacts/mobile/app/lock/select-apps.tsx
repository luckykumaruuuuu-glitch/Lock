        ...
        >
          <View style={styles.appRow}>
            ...
            {alreadyLocked ? (
              ...
              <View style={[styles.checkbox, { borderColor: selected ? "#FFEBD4" : "#3A3A3C" }]}>
                <Animated.View style={{ transform: [{ scale: checkScale }] }}>
                  {selected && (
                    <LinearGradient colors={["#FFEBD4", "#FFCF96"]} style={styles.checkFill}>
    ...
    <GradientBackground>
      ...
      {selection.selectedApps.length > 0 && (
        <View style={styles.selBanner}>
          <Feather name="check-circle" size={14} color="#FFEBD4" />
      ...
      <View style={[styles.footer, { paddingBottom: bottomPad + 20 }]}>
        <Pressable onPress={handleNext} disabled={selection.selectedApps.length === 0} style={({ pressed }) => [{ opacity: selection.selectedApps.length === 0 ? 0.35 : pressed ? 0.85 : 1 }]}>
          <LinearGradient colors={["#FFEBD4", "#FFCF96"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.nextBtn}>
...
const styles = StyleSheet.create({
  ...
  selText: { fontSize: 13, fontFamily: "Inter_600SemiBold", color: "#FFEBD4" },
  ...
  nextBtn: { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, paddingVertical: 17, borderRadius: 18, shadowColor: "#FFEBD4", shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.25, shadowRadius: 14, elevation: 10 },
