...
function ViewBoxInput({ label, value, onChange }: { label: string; value: string; onChange: (v: string) => void }) {
  const [vx, vy, vw, vh] = value.split(" ");
  ...
  return (
    <div className="mb-3">
      <div className="text-xs text-gray-400 mb-1 font-mono">{label} viewBox</div>
      <div className="grid grid-cols-4 gap-1">
        {[["x", vx, 0], ["y", vy, 1], ["w", vw, 2], ["h", vh, 3]].map(([lbl, val, idx]) => (
          <label key={lbl as string} className="flex flex-col items-center gap-0.5">
            <span className="text-[10px] text-gray-500">{lbl as string}</span>
            <input
              type="number"
              value={val as string}
              onChange={e => set(idx as number, e.target.value)}
              className="w-full bg-[#2a2a2a] text-white text-xs text-center rounded px-1 py-0.5 border border-[#444] focus:border-blue-500 outline-none"
            />
          </label>
        ))}
      </div>
...
function SizeInput({ label, w, h, onW, onH }: { label: string; w: number; h: number; onW: (v: number) => void; onH: (v: number) => void }) {
  return (
    <div className="mb-3">
      <div className="text-xs text-gray-400 mb-1 font-mono">{label} size</div>
      <div className="grid grid-cols-2 gap-1">
        <label className="flex flex-col items-center gap-0.5">
          <span className="text-[10px] text-gray-500">width</span>
          <input type="number" value={w} onChange={e => onW(num(e.target.value))}
            className="w-full bg-[#2a2a2a] text-white text-xs text-center rounded px-1 py-0.5 border border-[#444] focus:border-blue-500 outline-none" />
        </label>
        <label className="flex flex-col items-center gap-0.5">
          <span className="text-[10px] text-gray-500">height</span>
          <input type="number" value={h} onChange={e => onH(num(e.target.value))}
            className="w-full bg-[#2a2a2a] text-white text-xs text-center rounded px-1 py-0.5 border border-[#444] focus:border-blue-500 outline-none" />
        </label>
      </div>
    </div>
...
function BgInput({ label, value, onChange }: { label: string; value: string; onChange: (v: string) => void }) {
  return (
    <div className="mb-3">
      <div className="text-xs text-gray-400 mb-1 font-mono">{label} background</div>
      <div className="flex gap-2 items-center">
        <input type="color" value={value === "transparent" ? "#000000" : value}
          onChange={e => onChange(e.target.value)}
          className="w-8 h-7 rounded border border-[#444] cursor-pointer bg-transparent" />
        <input type="text" value={value} onChange={e => onChange(e.target.value)}
          className="flex-1 bg-[#2a2a2a] text-white text-xs rounded px-2 py-1 border border-[#444] focus:border-blue-500 outline-none font-mono" />
        <button onClick={() => onChange("transparent")}
          className="text-[10px] text-blue-400 hover:text-blue-300 whitespace-nowrap">transparent</button>
      </div>
    </div>
  );
}
...
export default function IconAdjuster() {
  // Google Pay
  const [gpayVB, setGpayVB] = useState("960 95 280 235");
  const [gpayW, setGpayW] = useState(36);
  const [gpayH, setGpayH] = useState(36);
  const [gpayBg, setGpayBg] = useState("transparent");
  const [gpayRadius, setGpayRadius] = useState(12);
  const [gpayOverflow, setGpayOverflow] = useState(false);

  // PhonePe
  const [ppVB, setPpVB] = useState("1040 96 280 272");
  const [ppW, setPpW] = useState(40);
  const [ppH, setPpH] = useState(40);
  const [ppBg, setPpBg] = useState("transparent");
  const [ppRadius, setPpRadius] = useState(12);
  const [ppOverflow, setPpOverflow] = useState(false);

  // UPI
  const [upiVB, setUpiVB] = useState("39 126 640 188");
  const [upiW, setUpiW] = useState(54);
  const [upiH, setUpiH] = useState(16);
  ...
  function copyCode() {
    const code = `// ── Google Pay ──────────────────────────────
// Container (icon.gpay):
//   backgroundColor: "${gpayBg}"
//   borderRadius: ${gpayRadius}
//   overflow: ${gpayOverflow ? '"hidden"' : 'undefined (not set)'}
// SVG: width={${gpayW}} height={${gpayH}} viewBox="${gpayVB}"

// ── PhonePe ──────────────────────────────────
// Container (icon.phonepe):
//   backgroundColor: "${ppBg}"
//   borderRadius: ${ppRadius}
//   overflow: ${ppOverflow ? '"hidden"' : 'undefined (not set)'}
// SVG: width={${ppW}} height={${ppH}} viewBox="${ppVB}"

// ── UPI Logo ─────────────────────────────────
// SVG: width={${upiW}} height={${upiH}} viewBox="${upiVB}"

// ── rowIcon (outer container) ─────────────────
//   borderRadius: ${containerRadius}
//   overflow: ${containerOverflow ? '"hidden"' : 'undefined (not set)'}
`;
  ...
  return (
    <div className="min-h-screen bg-[#111] text-white flex flex-col lg:flex-row gap-0">

      {/* ── Left: Live Preview ── */}
      <div className="flex-1 flex flex-col items-center justify-center p-6 min-h-[400px]">
        <div className="text-xs text-gray-500 mb-4 uppercase tracking-wider">Live Preview</div>

        {/* Payment sheet mockup */}
        <div className="w-[320px] bg-[#161616] rounded-2xl p-5 shadow-2xl">
          <div className="w-8 h-1 bg-[#3A3A3C] rounded mx-auto mb-4" />

          {/* Header: Pay by UPI */}
          <div className="flex items-center gap-2 mb-1">
            <span className="text-white font-bold text-lg">Pay by</span>
          ...
          <p className="text-[#8A8A8E] text-sm mb-4">Choose payment method</p>

          {/* Google Pay row */}
          <div className="bg-[#1E1E1E] rounded-xl px-3 py-4 flex items-center gap-3 mb-2">
            <div
              style={{
                width: 40,
                height: 40,
                borderRadius: containerRadius,
                overflow: containerOverflow ? "hidden" : "visible",
                display: "flex",
                alignItems: "center",
            ...
            >
              <div
                style={{
                  ...
                  borderRadius: gpayRadius,
                  backgroundColor: gpayBg,
                  overflow: gpayOverflow ? "hidden" : "visible",
                  display: "flex",
                  alignItems: "center",
                  justifyContent: "center",
                }}
              >
                <svg width={gpayW} height={gpayH} viewBox={gpayVB} style={{ display: "block" }}>
                  {GPAY_PATHS.map((p, i) => <path key={i} fill={p.fill} d={p.d} />)}
                </svg>
              </div>
            </div>
            <span className="text-white font-medium flex-1">Google Pay</span>
            ...
            >
              <div
                style={{
                  ...
                  borderRadius: ppRadius,
                  backgroundColor: ppBg,
                  overflow: ppOverflow ? "hidden" : "visible",
                  display: "flex",
                  alignItems: "center",
                  justifyContent: "center",
                }}
              >
                <svg width={ppW} height={ppH} viewBox={ppVB} style={{ display: "block" }}>
                  {PHONEPE_PATHS.map((p, i) => <path key={i} fill={p.fill} d={p.d} />)}
                </svg>
              </div>
            </div>
            <span className="text-white font-medium flex-1">PhonePe</span>
            <span className="text-[#666] text-sm">›</span>
          </div>

          {/* QR row */}
          <div className="bg-[#1E1E1E] rounded-xl px-3 py-4 flex items-center gap-3">
            ...
            <span className="text-white font-medium flex-1">Pay via QR Code</span>
            <span className="bg-[#34C759] text-white text-xs font-bold px-2 py-0.5 rounded-md mr-2">New</span>
            <span className="text-[#666] text-sm">›</span>
          </div>
        </div>
      </div>
      ...
      {/* ── Right: Controls ── */}
      <div className="w-full lg:w-[320px] bg-[#1a1a1a] border-l border-[#2a2a2a] p-5 overflow-y-auto">
        <div className="text-xs text-gray-500 uppercase tracking-wider mb-4">Adjust Controls</div>

        {/* Google Pay */}
        <div className="mb-5 pb-5 border-b border-[#2a2a2a]">
          <div className="text-sm font-semibold text-blue-400 mb-3">🔵 Google Pay</div>
          <ViewBoxInput label="GPay" value={gpayVB} onChange={setGpayVB} />
          <SizeInput label="GPay SVG" w={gpayW} h={gpayH} onW={setGpayW} onH={setGpayH} />
          <BgInput label="GPay container" value={gpayBg} onChange={setGpayBg} />
          <div className="mb-2">
            <div className="text-xs text-gray-400 mb-1 font-mono">GPay container borderRadius</div>
            <input type="range" min={0} max={20} value={gpayRadius} onChange={e => setGpayRadius(num(e.target.value))}
              className="w-full accent-blue-500" />
            <span className="text-xs text-gray-500">{gpayRadius}px</span>
          </div>
          <label className="flex items-center gap-2 text-xs text-gray-400 cursor-pointer">
            <input type="checkbox" checked={gpayOverflow} onChange={e => setGpayOverflow(e.target.checked)} />
            overflow: hidden on GPay container
          </label>
        </div>

        {/* PhonePe */}
        <div className="mb-5 pb-5 border-b border-[#2a2a2a]">
          <div className="text-sm font-semibold text-purple-400 mb-3">🟣 PhonePe</div>
          <ViewBoxInput label="PhonePe" value={ppVB} onChange={setPpVB} />
          <SizeInput label="PhonePe SVG" w={ppW} h={ppH} onW={setPpW} onH={setPpH} />
          <BgInput label="PhonePe container" value={ppBg} onChange={setPpBg} />
          <div className="mb-2">
            <div className="text-xs text-gray-400 mb-1 font-mono">PhonePe container borderRadius</div>
            <input type="range" min={0} max={20} value={ppRadius} onChange={e => setPpRadius(num(e.target.value))}
              className="w-full accent-purple-500" />
            <span className="text-xs text-gray-500">{ppRadius}px</span>
          </div>
          <label className="flex items-center gap-2 text-xs text-gray-400 cursor-pointer">
            <input type="checkbox" checked={ppOverflow} onChange={e => setPpOverflow(e.target.checked)} />
            overflow: hidden on PhonePe container
          </label>
        </div>

        {/* UPI */}
        <div className="mb-5 pb-5 border-b border-[#2a2a2a]">
          <div className="text-sm font-semibold text-orange-400 mb-3">🟠 UPI Logo</div>
          <ViewBoxInput label="UPI" value={upiVB} onChange={setUpiVB} />
          <SizeInput label="UPI SVG" w={upiW} h={upiH} onW={setUpiW} onH={setUpiH} />
        </div>

        {/* Outer rowIcon */}
        <div className="mb-5 pb-5 border-b border-[#2a2a2a]">
          <div className="text-sm font-semibold text-gray-300 mb-3">⬜ Outer Row Container</div>
          <div className="mb-2">
            <div className="text-xs text-gray-400 mb-1 font-mono">borderRadius</div>
            <input type="range" min={0} max={20} value={containerRadius} onChange={e => setContainerRadius(num(e.target.value))}
              className="w-full accent-gray-400" />
            <span className="text-xs text-gray-500">{containerRadius}px</span>
          </div>
          <label className="flex items-center gap-2 text-xs text-gray-400 cursor-pointer">
            <input type="checkbox" checked={containerOverflow} onChange={e => setContainerOverflow(e.target.checked)} />
        ...
        <button
          onClick={copyCode}
          className="w-full bg-blue-600 hover:bg-blue-500 active:bg-blue-700 text-white text-sm font-semibold py-2.5 rounded-xl transition-colors"
        >
          {copied ? "✓ Copied!" : "Copy Values to Clipboard"}
        </button>
        <p className="text-[10px] text-gray-600 text-center mt-2">
          Paste in chat — I'll apply exact values to the app
        </p>
      </div>
    </div>
  );
